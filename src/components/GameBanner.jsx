            ...
            <div className="relative border-4 border-accent rounded-xl overflow-hidden shadow-lg ">
               <div className="absolute bottom-0 p-5 bg-gradient-to-t from-slate-900 to-transparent w-full">
                  ...
                  <Link to={`/games/${game.id}`}>
                     <button className="bg-accent text-white px-2 p-1 rounded-xl">View game</button>
                  </Link>
               </div>
               {/* Game image */}
               <Link to={`/games/${game.id}`}>
                  <img className="w-full h-[400px] object-cover rounded-xl" src={game.background_image} alt={game.name} />
               </Link>
            </div>
         </div>
      </div>
