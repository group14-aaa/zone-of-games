...
const GamePage = () => {
  ...
  const fetchGameData = async (gId) => {
    try {
      const response = await rawgApi.getGameData(gId);
      // display data to the console
      console.log("Data from game page ", response.data);

      // Check if the data structure is an object with a 'data' property
      if (response.data) {
      ...
      console.error('Failed to fetch game data, error:', error);
    }
  }
  ...
  return (
    <div className="p-5">
      {/* game data here */}
      <h2 className='text-text'>{gameData.name}</h2>
      {/* other details */}
    </div>
  );
}
